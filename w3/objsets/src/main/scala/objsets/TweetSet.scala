...
abstract class TweetSet {
  ...
  def moreRetweeted(curr: Tweet): Tweet
...
class Empty extends TweetSet {
  ...
  override def moreRetweeted(curr: Tweet): Tweet = curr
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def descendingByRetweet: TweetList = {
    val head: Tweet = mostRetweeted
    new Cons(head, remove(head).descendingByRetweet)
  }

  override def mostRetweeted: Tweet = moreRetweeted(elem)

  def moreRetweeted(curr: Tweet): Tweet = {
    val toBeat: Tweet = if(curr.retweets > elem.retweets) curr else elem
    right.moreRetweeted(left.moreRetweeted(toBeat))
  }
