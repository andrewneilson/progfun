...
class ListsSuite extends FunSuite {
  ...
  test("sum incl negative numbers") {
    assert(sum(List(-1,-3,15,-11)) === 0)
  }
  ...
  test("max of a list incl. negative numbers") {
    assert(max(List(-10,-1,-5)) === -1)
  }

  test("max of a list incl. negative numbers in mixed order") {
    assert(max(List(-10,-1,-5)) === -1)
  }

  test("max of a list in ascending order with duplicates") {
    assert(max(List(5,5,10)) === 10)
  }

  test("max of a list of all the same number") {
    assert(max(List(1,1,1,1)) === 1)
  }
